#!/usr/bin/env python3
"""
JaegerBot Auto-Optimizer Scheduler
...
def run_optimization_python() -> bool:
    """Führt die Optimierung direkt via Python aus."""
    ...
    optimizer_path = SCRIPT_DIR / "src" / "jaegerbot" / "analysis" / "optimizer.py"
    ...
    try:
        ...
        if returncode == 0:
            ...
            if opt_settings.get("send_telegram_on_completion", True):
                config_dir = SCRIPT_DIR / "src" / "jaegerbot" / "strategy" / "configs"
                ...
                send_telegram(f"""✅ JaegerBot Auto-Optimierung ABGESCHLOSSEN
...
Nächste Optimierung in {interval_days} Tagen.""")
            ...
            if opt_settings.get("send_telegram_on_completion", True):
                send_telegram(f"""❌ JaegerBot Auto-Optimierung FEHLGESCHLAGEN
...
def main():
    parser = argparse.ArgumentParser(description="JaegerBot Auto-Optimizer Scheduler")
    parser.add_argument("--daemon", action="store_true", help="Als Hintergrund-Service laufen")
    parser.add_argument("--check-only", action="store_true", help="Nur prüfen ob Optimierung fällig ist")
    parser.add_argument("--force", action="store_true", help="Optimierung sofort starten")
    parser.add_argument("--interval", type=int, default=60, help="Prüf-Intervall in Sekunden")
