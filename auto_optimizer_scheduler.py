...
die Pipeline aus (LSTM-Training + Hyperparameter-Optimierung).
...
def _resolve_pairs_auto(live_settings: dict) -> list:
    ...
    for s in live_settings.get('active_strategies', []):
        ...
        if sym and tf and (sym, tf) not in seen:
            pairs.append((sym, tf))
            seen.add((sym, tf))
    return pairs or [('BTC/USDT:USDT', '4h'), ('ETH/USDT:USDT', '1h')]
...
def _send_end_telegram(elapsed_seconds: float):
    ...
    if not os.path.exists(OPTIMIZER_RESULTS_FILE):
        _send_telegram_plain(f"\u2705 JaegerBot Auto-Optimizer abgeschlossen\nDauer: {dur}")
        return
    ...
    try:
        ...
        _send_telegram_plain(f"\u2705 JaegerBot Auto-Optimizer abgeschlossen (Dauer: {dur})")
        return
    ...
    lines = [f"\u2705 JaegerBot Auto-Optimizer abgeschlossen (Dauer: {dur})"]
